<template>
  <div class="card-wrapper">
    <div class="card" @click="handleView($event)" :class="{'open': item.open}" :style="styleObject">
      <div class="card-top">
        <div class="img"
  ...
  methods: {
    handleView(el) {
      ...
      if(this.item.open) {
        ...
        window.scrollY(0, parseInt(scrollY) * -1);
      }
    }
  }
};
...
.card {
  ...
  cursor: pointer;
  z-index: 1;
  transition: all .5s cubic-bezier(.68,0,.4,1.35);
}
.open {
  z-index: 100;
  border-radius: 0;
